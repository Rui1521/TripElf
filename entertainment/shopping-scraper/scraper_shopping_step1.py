...
def crawl_page(category, zipcode, page_num, proxy, verbose=False):
    ...
    for r in restaurants:
        ...
        try:
            ...
            if len(secondaryAttributes) == 3:
                district = secondaryAttributes[0].getText().replace(
                    ',', ' ')
                addr = secondaryAttributes[1].getText().replace(',', ' ')
                phone = secondaryAttributes[2].getText().replace(',', ' ')
            else:
                if secondaryAttributes[1].getText()[0] == '(':
                    addr = secondaryAttributes[0].getText().replace(',', ' ')
                    phone = secondaryAttributes[1].getText().replace(',', ' ')
                else:
                    district = secondaryAttributes[0].getText().replace(',', ' ')
                    addr = secondaryAttributes[1].getText().replace(',', ' ')
        except Exception as e:
    ...
    try:
        assert(len(extracted) == 10) # restaurants: 30; shopping: 10; nightlife: 30
    except AssertionError as e:
...
def crawl():
    ...
    category = 'shopping' # restaurants, shopping, nightlife
    ...
    with open('./yelp_'+category+'_listings.csv', 'w') as yelp_listings:
        ...
        for zipcode in some_zipcodes:
            ...
            while flag:
                ...
                for listing in extracted:
                    yelp_listings.write('\n'+listing)
                page += 10  # restaurants: 30; shopping: 10; nightlife: 30
